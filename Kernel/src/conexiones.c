...
void levantar_conexion_cpu_dispatch(char* ipServer, char* portServer, t_log* logger)
{
	...
	log_info(logger, "Se envia Handshake a CPU dispatch");

	uint8_t respuesta = 0;
	recv(socket_cpu_dispatch, &respuesta, sizeof(uint8_t), 0);
	log_info(logger, "Mensaje recibido del CPU:  %d", respuesta);

	if(respuesta == ACEPTAR_CONEXION_DISPATCH)
		log_info(logger, "Conexion establecida con la CPU dispatch");
	else
	{
		log_error(logger, "No se puede establecer conexion con CPU dispatch. El server contactado responde con un mensaje no predeterminado: %d.", respuesta);
		exit(EXIT_FAILURE);
	}
...
void levantar_conexion_cpu_interrupt(char* ipServer, char* portServer, t_log* logger)
{
	...
	log_info(logger, "Se envia Handshake a la CPU interrupt");

	uint8_t respuesta = 0;
	recv(socket_cpu_interrupt, &respuesta, sizeof(uint8_t), 0);
	log_info(logger, "Mensaje recibido de la CPU interrupt:  %d", respuesta);

	if(respuesta == ACEPTAR_CONEXION_INTERRUPT)
		log_info(logger, "Conexion establecida con la CPU interrupt");
	else
	{
...
void levantar_conexion_memoria(char* ipServer, char* portServer, t_log* logger)
{
	...
	log_info(logger, "Mensaje recibido de la memoria:  %d", respuesta);

	if(respuesta == ACEPTAR_CONEXION_KERNEL)
		log_info(logger, "Conexion establecida con la memoria");
	else
	{
		log_error(logger, "No se puede establecer conexion con la memoria. El server contactado responde con un mensaje no predeterminado: %d.", respuesta);
		exit(EXIT_FAILURE);
	}
}
